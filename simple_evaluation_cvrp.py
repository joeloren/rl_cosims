...
def main():
    ...
    # in this case each time we choose a different action each time since the action is the actual index of the customer
    sim.reset()
    ...
    reward, done = sim.step(1)
    print(f"reward {reward}, done {done}")
    available_customers = sim.get_available_customers()
    print(f"available customers:{available_customers}")
    reward, done = sim.step(2)
    print(f"reward {reward}, done {done}")
    available_customers = sim.get_available_customers()
    print(f"available customers:{available_customers}")
    ...
    print(f"reward {reward}, done {done}")
    available_customers = sim.simulator.get_available_customers()
    print(f"available customers:{available_customers}")
    obs, reward, done, _ = sim.step(0)
    print(f"reward {reward}, done {done}")
    available_customers = sim.simulator.get_available_customers()
