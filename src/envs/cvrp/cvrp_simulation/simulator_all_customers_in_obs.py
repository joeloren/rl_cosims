...
class CVRPSimulation(Env):
    ...
    def __init__(self, max_customers: int, problem_generator, allow_noop=True) -> None:
        ...
        obs_spaces = {
            ...
            "customer_visited": spaces.MultiBinary(max_customers),
    ...
    def current_state_to_observation(self) -> dict:
        ...
        customer_visited = deepcopy(self.current_state.customer_visited[opened_customers_ids])
    ...
    def reset_future(
        ...
        copy_env.initial_state = State(
            ...
            current_vehicle_customer=copy_env.initial_state.current_vehicle_customer
        )
        ...
        copy_env.current_state = State(
            ...
            customer_ids=customer_ids,
            customer_visited=customer_visited,
            current_vehicle_customer=copy_env.current_state.current_vehicle_customer
        )
        return copy_env

    def get_opened_customers(self) -> np.ndarray:
        """
        this function returns the ids of the opened customers (visited and not visited)
        :return: np.ndarray of available customer id's, length of array is the number of
        customers that are available to observation (visited and not visited)
        """
        ...
        opened_customer_ids = self.current_state.customer_ids[self.current_state.customer_times <= self.current_time]
        return opened_customer_ids

    def get_opened_and_not_visited_customers(self) -> np.ndarray:
        """
    ...
    def get_masked_options(self, opened_customers_ids: np.ndarray) -> np.ndarray:
        ...
        # if opened_customers_not_visited = True the customer has not been visited yet and is a feasible action
        opened_customers_not_visited = np.logical_not(self.current_state.customer_visited[opened_customers_ids])
        num_opened_customers = opened_customers_ids.size
        masked_options = np.ones(num_opened_customers + 2).astype(np.bool)
        masked_options[:-2] = np.logical_and(opened_customer_demands <= self.current_state.current_vehicle_capacity,
                                             opened_customers_not_visited)
