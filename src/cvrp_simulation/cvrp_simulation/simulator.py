...
class State:
    """
    this class defines the cvrp_simulation state at every time step. this state describes the total
...
class CVRPSimulation(Env):
    ...
    def __init__(self, max_customers: int, problem_generator, allow_noop=True) -> None:
        ...
        self.current_state = None  # current state of the cvrp_simulation, this is updated at every step() call
        ...
        self.allow_noop = allow_noop  # if noop action is allowed in cvrp_simulation
        self.NOOP_INDEX = -1
        self.DEPOT_INDEX = -2
    ...
    def step(self, action_chosen: int) -> (dict, float, bool, dict):
        ...
        # if cvrp_simulation is done, add returning to depot to reward
        if is_done:
    ...
    def reset_future(
        ...
        :return: copy_env: updated cvrp_simulation with new customers
        """
        # print(f"resetting future for cvrp_simulation , t:{self.current_time}")
        ...
        if horizon_limit is None and n_future_customers is not None:
            ...
            raise ValueError("in future_reset horizon_limit and n_future customers are both not equal to None, "
                             "cvrp_simulation does not know what how many future customers to create")
    ...
    def get_next_time(self) -> int:
        ...
        if future_customer_times.size > 0:
            ...
            # to do decide if the cvrp_simulation should send an exception or something else should
            #  happen in this case
